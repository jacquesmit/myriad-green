  ...
  const resolver = new PaymentResolver({
    lookupService: {
      ...
      async findEntityByCanonicalId(type, id) {
        return type === 'Invoice' && id === 'INV-1' ? invoice() : null;
      }
    }
  });
...
test('payment larger than exact invoice balance becomes conflict/review', async () => {
  ...
  const resolver = new PaymentResolver({
    lookupService: {
      async findPaymentEventByProviderTransactionId() { return null; },
      async findEntityByCanonicalId() { return invoice('R 1,000.00'); }
    }
  });
...
test('invoice document number can be used as exact reference', async () => {
  ...
  const resolver = new PaymentResolver({
    lookupService: {
      async findPaymentEventByProviderTransactionId() { return null; },
      async findInvoiceByDocumentNo(number) {
        return number === 'MRG-INV-TEST-1' ? invoice() : null;
      }
    }
  });
...
test('control-only match may record cleared event but cannot infer allocation', async () => {
  const env = event({
    invoice_id: '',
    control_id: 'PAY-1',
    clearance_authority: 'BENEFICIARY_BANK',
  ...
  const resolver = new PaymentResolver({
    lookupService: {
      async findPaymentEventByProviderTransactionId() { return null; },
      async findEntityByCanonicalId(type, id) {
        return type === 'PaymentControl' && id === 'PAY-1'
          ? {
              'Control ID': 'PAY-1',
...
test('owner-verified bank evidence may be CLEARED without inventing provider transaction ID', async () => {
  ...
  const resolver = new PaymentResolver({
    lookupService: {
      async findPaymentEventByProviderTransactionId() { return null; },
      async findEntityByCanonicalId() { return invoice(); }
    }
  });
